...
<form action="{{ $action ?? '' }}" method="{{ $formMethod ?? 'post'}}" id="{{ $id ?? '__form__request' }}"
      @if($viewMode == 'show') class="show-mode" @endif>
    ...
    <div class="card-body loading-content">
        ...
        $tabLocalId = Str::singular($tabId);
        ?>

        <div class="bordered-tabs {{ $attributes['class'] ?? '' }}">
            <ul class="nav nav-tabs" id="{{$tabId}}" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="general" data-bs-toggle="tab" href="#{{$tabLocalId}}__general"
                       role="tab" aria-controls="home"
                       aria-selected="true">{{__('__dashboard.tab.general')}}</a>
                </li>
                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                    <li class="nav-item">
                        <a class="nav-link" id="{{$localeCode}}" data-bs-toggle="tab"
                           href="#{{$tabLocalId}}__{{$localeCode}}" role="tab" aria-controls="home"
                           aria-selected="true">{{ $properties['name'] }}</a>
                    </li>
                @endforeach
            </ul>
            <div class="tab-content {{ $attributes['tabContentClass'] ?? '' }}" id="{{$tabId}}__content">
                <div class="tab-pane fade show active" id="{{$tabLocalId}}__general" role="tabpanel"
                ...
                @else
                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        <div class="tab-pane {{ $attributes['tabsClass'] ?? '' }} fade"
                             id="{{$tabLocalId}}__{{$localeCode}}" role="tabpanel" aria-labelledby="{{$localeCode}}">

                            {{  $renderMlHtml($mlTabsData, $localeCode, $attributes['mlData'] ?: []) }}
                            @if($mlTabsData->attributes['addCopyButtons'] === null)
                                <div class="form-group">
                                    <div class="copy-ml-buttons">
                                        @foreach(getSupportedLocales() as $supportedLocal)
                                            ...
                                            <button type="button" class="btn btn-success copy-ml-info-btn"
                                                    ...
                                                    data-to-lang-code="{{$supportedLocal}}">{{trans('button.info.copy.to.'.$supportedLocal)}}</button>
                                        @endforeach
                                    </div>
                                </div>
                            @endif
